-- =================================================================
-- QUALIFIED ACTIVE MERCHANTS per CALENDAR MONTH (1+ / 5+ / 10+ receipts)
-- READ-ONLY (single SELECT, works with DATA_VIEWER)
-- =================================================================
-- The dashboard's "active merchants" counts a merchant with ONE receipt in the month the
-- same as one with four hundred. That is the right top-line number and the wrong one for
-- asking whether a merchant is actually running its business on the POS. This adds the two
-- higher bars beside it: 5+ and 10+ receipts in the month.
--
-- THE 1+ COLUMN MUST REPRODUCE THE EXISTING SERIES EXACTLY. It is drawn on the same chart
-- as the number the page already publishes, so if the two disagree the chart contradicts
-- itself. Every filter below is therefore copied from receipts_tpv_daily_asof.sql, which is
-- where activeMonthly comes from:
--   * the same source table, LOYVERSE_RECEIPTS_UNIQUE
--   * the same receipt filters — positive money, not cancelled, not a refund
--   * the same merchant master — email and created_at present, country present, and the
--     shared bot/fake-account exclusion, which is lifted verbatim below
-- The build step reconciles the 1+ column against kpi-data.js and fails if they diverge.
--
-- WHAT IS DELIBERATELY NOT COPIED: the as-of snapshot spine. That query fans every receipt
-- across ~70 daily snapshots to keep cumulative cohort counts monotonic, which is why it is
-- the slowest thing in the pipeline and the only one that has ever hit the 3600s timeout.
-- Nothing here needs it: a calendar month's receipt count does not depend on when you ask.
-- One scan, one group-by, ~50 rows out.
--
-- RECEIPTS, NOT VOLUME. A merchant ringing up 10 small sales is more alive than one with a
-- single large invoice, and the question is whether the till is in daily use.
-- =================================================================
...
merchant_master AS (
    -- Same population the active series is drawn from: real merchants, bots excluded.
    SELECT LOYVERSE_ID
    FROM LOYVERSE_DATA_LAKE.PUBLIC.LOYVERSE_MERCHANTS
...
--
-- AGGREGATE FIRST, JOIN AFTER. The obvious shape — join the merchant master inside this
-- scan to drop bots — makes Snowflake shuffle billions of receipt rows against the
-- merchant table before it can reduce anything. The group-by below collapses ~9.6B
-- receipts to a few million merchant-months first, and the merchant filter is applied to
-- THAT. Same answer, a fraction of the work.
--
-- DATES ARE COMPARED AS TEXT, ON PURPOSE. RECEIPT_DATE is TEXT holding ISO-8601 with a Z
-- suffix ("2026-05-06T06:25:05.000Z"), which sorts lexicographically in date order, so a
-- string range does the same job as parsing. The first seven characters ARE the calendar
-- month, so bucketing is a substring rather than a parse. The table is 9.6B rows and 2.7TB
-- with NO clustering key, so nothing can be pruned and every row is read whatever we do;
-- what this avoids is running TRY_TO_TIMESTAMP nine billion times. The build reconciles the
-- 1+ column against the published series, so if the format assumption ever breaks it fails
...
receipts_per_merchant_month AS (
    ...
    WHERE r.TOTAL_MONEY IS NOT NULL AND r.TOTAL_MONEY > 0
      ...
      -- Device-clock rows. Loyverse tills stamp receipts from the device and a handful
      -- carry dates centuries out (one reads 2152-01-02), so the window is bounded at both
      -- ends rather than trusted. The upper bound is the FIRST DAY OF NEXT MONTH, not a
      -- month from today: today-plus-a-month let device-clock rows dated early next month
      -- create a phantom trailing bar.
      AND r.RECEIPT_DATE >= '2021-01-01'
      AND r.RECEIPT_DATE <  TO_VARCHAR(DATE_TRUNC('MONTH', DATEADD('month', 1, CURRENT_DATE())), 'YYYY-MM-DD')
    GROUP BY 1, 2
)
SELECT a.CALENDAR_MONTH                    AS MONTH,
       COUNT(*)                            AS ACTIVE_1,
       COUNT_IF(a.N_RECEIPTS >= 5)         AS ACTIVE_5,
       COUNT_IF(a.N_RECEIPTS >= 10)        AS ACTIVE_10,
       SUM(a.N_RECEIPTS)                   AS RECEIPTS
FROM receipts_per_merchant_month a
JOIN merchant_master m ON m.LOYVERSE_ID = a.MERCHANT_ID
GROUP BY 1
ORDER BY 1;
