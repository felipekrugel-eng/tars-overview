...
--   GRACE  = same, but PERIOD_END is extended by GRACE_DAYS, so a renewal that posts a few
--            days after the old period ends does NOT register as a churn + re-add.
...
WITH params AS (
    SELECT DATE '2022-01-01' AS D_START,   -- full history from Jan 2022
           ...
           3                 AS GRACE_DAYS  -- try 3, 5, 7 to see sensitivity
),
...
active_merchants AS (SELECT DISTINCT EMAIL_KEY FROM member_grace),   -- grace is the superset
...
chg AS (
    SELECT EMAIL_KEY, d, is_raw, is_grace,
