...
merchant_gtv AS (
  SELECT
    ...
    SUM(lr.TOTAL_MONEY * fx.rate) AS gtv_30d_eur  -- NULL rate excludes unknown currencies
...
merchant_gtv_alltime AS (
  SELECT
    lr.MERCHANT_ID AS loyverse_id,
    SUM(lr.TOTAL_MONEY * fx.rate) AS gtv_total_eur  -- NULL rate excludes unknown currencies
  FROM LOYVERSE_RECEIPTS lr
  LEFT JOIN fx_rates fx ON fx.currency = COALESCE(lr.CURRENCY, 'EUR')
  GROUP BY lr.MERCHANT_ID
),
