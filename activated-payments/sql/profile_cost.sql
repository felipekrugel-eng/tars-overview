...
-- amounts to accept: on the scouting run American Express landed near 60bp against Visa debit
-- near 20bp, so a shift in mix moves the take rate without anything else changing.
--
...
-- the charge amount repeats that amount once per fee row. The scouting query did exactly that and
-- reported $1,569,139 of Visa-debit TPV against a real total of $424,163 — roughly 3.7x — which
-- makes every cost-in-bps figure it printed far too low. Those numbers must not be reused.
--
